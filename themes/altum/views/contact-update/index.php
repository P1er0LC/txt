    ...
    <?php if(settings()->main->breadcrumbs_is_enabled): ?>
        <nav aria-label="breadcrumb">
            <ol class="custom-breadcrumbs small">
                ...
                <li>
                    <a href="<?= url('contact-view/' . $data->contact->contact_id) ?>"><?= l('contact.breadcrumb') ?></a><i class="fas fa-fw fa-angle-right"></i>
                </li>
                <li class="active" aria-current="page"><?= l('contact_update.breadcrumb') ?></li>
            </ol>
        </nav>
    <?php endif ?>

    <div class="d-flex justify-content-between mb-4">
        <h1 class="h4 text-truncate mb-0"><i class="fas fa-fw fa-xs fa-address-book mr-1"></i> <?= l('contact_update.header') ?></h1>
        ...
        <div class="card-body">
            ...
            <form id="form" action="" method="post" role="form" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="phone_number" name="phone_number" class="form-control <?= \Altum\Alerts::has_field_errors('phone_number') ? 'is-invalid' : null ?>" value="<?= $data->contact->phone_number ?>" placeholder="<?= l('contacts.phone_number_placeholder') ?>" required="required" />
                    <?= \Altum\Alerts::output_field_error('phone_number') ?>
                </div>

                <div class="form-group">
                    <label for="name"><i class="fas fa-fw fa-sm fa-signature text-muted mr-1"></i> <?= l('global.name') ?></label>
                    <input type="text" id="name" name="name" class="form-control <?= \Altum\Alerts::has_field_errors('name') ? 'is-invalid' : null ?>" value="<?= $data->contact->name ?>" />
