...
class Index extends Controller {
    ...
    public function index() {
        ...
        if(is_null($cache_instance->get())) {
            ...
            $total_sent_sms = database()->query("SELECT MAX(`sms_id`) AS `total` FROM `sms`")->fetch_object()->total ?? 0;
            $total_devices = database()->query("SELECT MAX(`device_id`) AS `total` FROM `devices`")->fetch_object()->total ?? 0;
            $total_contacts = database()->query("SELECT MAX(`contact_id`) AS `total` FROM `contacts`")->fetch_object()->total ?? 0;
