...
class AdminSettings extends Controller {
    ...
    public function custom_images() {
        ...
        if(!empty($_POST)) {
            ...
            $images_keys = [
                'index/hero-light.webp',
                'index/hero-dark.webp',
                'index/sms.webp',
                'index/contacts.webp',
                'index/campaigns.webp',
                'index/flows.webp',
                'index/segments.webp',
                'index/rss_automations.webp',
                'index/recurring_campaigns.webp',
    ...
    public function license() {
        ...
        if(!empty($_POST) && !empty($_POST['new_license'])) {
            ...
            $bypass_license = true; // ⚠️ Cambiar a false en producción
            
            if($bypass_license) {
                $response = new \stdClass();
                $response->body = new \stdClass();
                $response->body->status = 'success';
                $response->body->message = 'License updated (development mode)';
                $response->body->sql = '';
            } else {
    ...
    public function support() {
        ...
        if(!empty($_POST) && !empty($_POST['new_key'])) {
            ...
            if($bypass_license) {
                ...
                $response->body = new \stdClass();
                $response->body->status = 'success';
                $response->body->message = 'Support key validated (development mode)';
                $response->body->sql = '';
            } else {
    ...
    public function sms() {
        ...
        if(!empty($_POST)) {
            ...
            settings()->sms->apk = \Altum\Uploads::process_upload(settings()->sms->apk, 'apk', 'apk', 'apk_remove', null);
            $_POST['branding'] = trim($_POST['branding']);
            $_POST['email_notices_is_enabled'] = (int) isset($_POST['email_notices_is_enabled']);
            $_POST['firebase_project_id'] = trim($_POST['firebase_project_id']);
            settings()->sms->firebase_service_account_json = \Altum\Uploads::process_upload(settings()->sms->firebase_service_account_json, 'firebase', 'firebase_service_account_json', 'firebase_service_account_json_remove', null);

            $value = json_encode([
                'app_name' => $_POST['app_name'],
                'apk' => settings()->sms->apk,
                'branding' => $_POST['branding'],
                'email_notices_is_enabled' => $_POST['email_notices_is_enabled'],

                'firebase_project_id' => $_POST['firebase_project_id'],
                'firebase_service_account_json' => settings()->sms->firebase_service_account_json,

                'scheduled_and_flows_sms_per_cron' => (int) $_POST['scheduled_and_flows_sms_per_cron'],
                'campaigns_sms_inserts_per_cron' => (int) $_POST['campaigns_sms_inserts_per_cron'],
                'flows_contacts_per_cron' => (int) $_POST['flows_contacts_per_cron'],
                'rss_automations_per_cron' => (int) $_POST['rss_automations_per_cron'],
                'recurring_campaigns_per_cron' => (int) $_POST['recurring_campaigns_per_cron'],
            ]);

            $this->update_settings('sms', $value);
        }
    }
