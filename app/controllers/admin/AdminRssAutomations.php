...
class AdminRssAutomations extends Controller {
    ...
    public function index() {
        ...
        $rss_automations_result = database()->query("
            SELECT
                `rss_automations`.*, `users`.`name` AS `user_name`, `users`.`email` AS `user_email`, `users`.`avatar` AS `user_avatar`
            FROM
                `rss_automations`
            LEFT JOIN
            ...
            switch($_POST['type']) {
                case 'delete':
                    ...
                    foreach($_POST['selected'] as $rss_automation_id) {
                        if($rss_automation = db()->where('rss_automation_id', $rss_automation_id)->getOne('flows')) {
    ...
    public function delete() {
        ...
        if(!$rss_automation = db()->where('rss_automation_id', $rss_automation_id)->getOne('rss_automations', ['rss_automation_id', 'user_id', 'name'])) {
            redirect('admin/rss-automations');
        }
