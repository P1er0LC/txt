...
class AdminStatistics extends Controller {
    ...
    protected function rss_automations() {

        $total = ['rss_automations' => 0];

        /* Data */
        $convert_tz_sql = get_convert_tz_sql('`datetime`', $this->user->timezone);

        $rss_automations_chart = [];
        $result = database()->query("
            SELECT
                COUNT(*) AS `total`,
                DATE_FORMAT({$convert_tz_sql}, '{$this->datetime['query_date_format']}') AS `formatted_date`
        ...
        while($row = $result->fetch_object()) {
            ...
            $rss_automations_chart[$row->formatted_date] = [
                'rss_automations' => $row->total
            ];

            $total['rss_automations'] += $row->total;
        }

        $rss_automations_chart = get_chart_data($rss_automations_chart);

        return [
            'total' => $total,
            'rss_automations_chart' => $rss_automations_chart,
        ];

    }

    protected function recurring_campaigns() {

        $total = ['recurring_campaigns' => 0];

        /* Data */
        $convert_tz_sql = get_convert_tz_sql('`datetime`', $this->user->timezone);

        $recurring_campaigns_chart = [];
        $result = database()->query("
            SELECT
                COUNT(*) AS `total`,
                DATE_FORMAT({$convert_tz_sql}, '{$this->datetime['query_date_format']}') AS `formatted_date`
        ...
        while($row = $result->fetch_object()) {
            ...
            $recurring_campaigns_chart[$row->formatted_date] = [
                'recurring_campaigns' => $row->total
            ];

            $total['recurring_campaigns'] += $row->total;
        }

        $recurring_campaigns_chart = get_chart_data($recurring_campaigns_chart);

        return [
            'total' => $total,
            'recurring_campaigns_chart' => $recurring_campaigns_chart,
        ];
    ...
    protected function devices() {
        ...
        while($row = $result->fetch_object()) {
            ...
            $devices_chart[$row->formatted_date] = [
                'devices' => $row->total
            ];
