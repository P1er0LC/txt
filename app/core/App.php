...
class App {
    ...
    public function __construct() {
        ...
        if(is_logged_in()) {
            ...
            if($user->plan_settings->campaigns_per_month_limit != -1 && $user->text_campaigns_current_month > $user->plan_settings->campaigns_per_month_limit && !isset($_COOKIE['plan_campaigns_limit_notice'])) {
                Alerts::add_warning('<strong>' . l('global.notifications.user_campaigns_limit.title') . '</strong>' . (settings()->payment->is_enabled ? '<br />' . sprintf(l('global.notifications.user_campaigns_limit.description'), '<a href="' . url('plan') . '" class="font-weight-bold">', '</a>') : null));
                setcookie('plan_campaigns_limit_notice', 1, time()+60*60*24*1, COOKIE_PATH);
            }

            if($user->plan_settings->campaigns_per_month_limit != -1 && $user->text_sent_sms_current_month > $user->plan_settings->campaigns_per_month_limit && !isset($_COOKIE['plan_sent_sms_limit_notice'])) {
                Alerts::add_warning('<strong>' . l('global.notifications.user_sent_sms_limit.title') . '</strong>' . (settings()->payment->is_enabled ? '<br />' . sprintf(l('global.notifications.user_sent_sms_limit.description'), '<a href="' . url('plan') . '" class="font-weight-bold">', '</a>') : null));
                setcookie('plan_sent_sms_limit_notice', 1, time()+60*60*24*1, COOKIE_PATH);
            }
        }
...
//        \Altum\Authentication::$user->email = 'hello@example.com';
//        \Altum\Authentication::$user->name = 'Example';
